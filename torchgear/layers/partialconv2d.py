...
class PartialConv2d(nn.Conv2d):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None or self.last_size != (x.data.shape[2], x.data.shape[3]):
            self.last_size = (x.data.shape[2], x.data.shape[3])
            ...
            with torch.no_grad():
                ...
                if mask is None:
                    ...
                    if self.multi_channel:
                        mask = torch.ones(x.data.shape[0], x.data.shape[1],
                                          x.data.shape[2], x.data.shape[3]).to(x)
                    else:
        ...
        if self.update_mask.type() != x.type() or self.mask_ratio.type() != x.type():
            self.update_mask.to(x)
            self.mask_ratio.to(x)
